...
(function () {
  ...
  function initGreeting() {
    ...
    function parentLine(father, mother, fatherDeceased, motherDeceased) {
      ...
      return `<span class="${fd}">${father}</span> 
        <span class="parent-dot">●</span> <span class="${md}">${mother}</span>`;
    }

    const parentsHTML = `
      <div class="parent-row">
        ${parentLine(g.father, g.mother, g.fatherDeceased, g.motherDeceased)}
        <span class="parent-label">의 아들</span> <span class="child-name">${g.name}</span>
      </div>
      <div class="parent-row is-bride">
        ${parentLine(b.father, b.mother, b.fatherDeceased, b.motherDeceased)}
        <span class="parent-label">의 딸</span> <span class="child-name">${b.name}</span>
      </div>
    `;
  ...
  function initBusForm() {
    const openBtn = $('#busApplyBtn');
    const modal = $('#busModal');
    const backdrop = $('#busModalBackdrop');
    const closeBtn = $('#busModalClose');
    const form = $('#busForm');
    const nameInput = $('#busName');
    const phoneInput = $('#busPhone');
    const messageInput = $('#busMessage');
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (!name || !phone) {
        showToast('성함과 연락처를 모두 입력해주세요.');
        return;
      }

      const endpoint = import.meta.env.VITE_BUS_FORM_ENDPOINT;
      if (!endpoint) {
        showToast('버스 신청 경로가 아직 설정되지 않았습니다.');
        return;
      }
      ...
      try {
        ...
        await fetch(endpoint, {
          ...
          body: JSON.stringify({ kind: 'bus', name, phone, message })
        });

        showToast('전세버스 신청이 완료되었습니다. 감사합니다.');
        form.reset();
        closeModal();
      } catch (err) {
        console.error('버스 신청 오류:', err);
        showToast('신청 처리 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.');
      } finally {
  ...
  function initRsvpForm() {
    const openBtn = $('#rsvpApplyBtn');
    const modal = $('#rsvpModal');
    const backdrop = $('#rsvpModalBackdrop');
    const closeBtn = $('#rsvpModalClose');
    const form = $('#rsvpForm');
    ...
    const statusButtons = $$('.rsvp-status-toggle .rsvp-toggle-btn', modal);
    const partyButtons = $$('.rsvp-party-toggle .rsvp-toggle-btn', modal);
    ...
    if (partyButtons.length && partyHidden) {
      partyButtons.forEach((btn) => {
        btn.addEventListener('click', () => {
          ...
          btn.classList.add('is-active');
          partyHidden.value = btn.dataset.party || '';
        });
      });
    }
    ...
    form.addEventListener('submit', async (e) => {
      ...
      const endpoint = import.meta.env.VITE_BUS_FORM_ENDPOINT;
      if (!endpoint) {
        showToast('참석 여부 저장 경로가 아직 설정되지 않았습니다.');
        return;
      }
      ...
      try {
        ...
        showToast('전송 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.');
      } finally {
        submitBtn && (submitBtn.disabled = false);
      }
    });
  ...
  async function fetchBusCount() {
    ...
    try {
      ...
      if (!sheetId || !apiKey) {
        statusEl.textContent = '버스 정보 설정이 되어 있지 않습니다. (.env 파일을 확인해주세요)';
        return;
      }
      ...
      statusEl.textContent = `현재까지 신청 인원: ${total}명`;
    } catch (error) {
      console.error('버스 인원 조회 오류:', error);
      statusEl.textContent = '버스 인원 정보를 불러오지 못했습니다. 나중에 다시 확인해주세요.';
    }
  }
